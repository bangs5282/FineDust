...
void CFineDustDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Text(pDX, IDC_STATIC_TEST, m_strTest);
}
...
long CFineDustDlg::OnGETAPI(WPARAM wParam, LPARAM lParam) {
	SendRequestOpenAPI();
	SetBackgroundColor(RGB(a, b, c));
	if (realchange == 1) {
		Invalidate();
...
int CFineDustDlg::SendRequestOpenAPI()
{
	...
	if (pFile) {
		...
		AfxMessageBox(_T("API를 받아오는데 실패했습니다!"));
	...
	if (test != -1) {
		...
		temp = strData.Find(_T("<"));
		if (test == 0) {
			//m_strTest = strData.Mid(1, temp - 1);
		    CRect rect;
